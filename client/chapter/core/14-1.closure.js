...
function first(){
  ...
  //return second() //함수 실행. 결과: 30
  return second //함수 본문이 담김. result가 second 함수처럼 동작. rusult()
}

const result = first(); // first()(20) 이렇게 second 매게변수에 값 보내줄 수 있는데 이러면 보기 불편.(커링 펑션션) 그래서 보통 이렇게 변수에 first를 담아서 result를 second로 만들어준 후 값 전달함.
//함수는 태어난 환경을 기억하는데 그래서 second가 튀어 나가도 second는 태어난 환경인 first 속 변수 x를 기억한다.
//참조가 끝나면 가비지 컬렉터가 수집해가는데 first를 계속 참조하기 때문에 수집해가지 않음.(계속 기억)
//클로저가 메모리 수집을 못해가기 때문에 많이 쓰게되면 메모리 문제 -> 근데 그 정도가 많음. 만개정도?

//안쪽에 있는 대상이 바깥쪽에 접근해서 사용해야 클로저.
