...
const iter = friends[Symbol.iterator]() //이터러블한 객체로 만들어줌.
//이터러블 객체=> 장점: 내가 원하는 만큼만 반복하도록 할 수 있음. 성능 최적화.
//iterator 내장하고 있으면 이터러블 요소. 이터러블한 객체로 쓰려면 위에 코드를 따로 써줘야 함.
...
for(const name of iter){ //위에서 iter을 한번 실행했기 때문에 iter 이 없음. =>이터러블 객체는 순환을 한번 하고 나면 더이상 그 아이템을 꺼내 쓸 수 없음.
  console.log(name);
}
